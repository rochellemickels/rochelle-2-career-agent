...
USER_AGENT = "RochelleCareerAgent/0.1 (+https://github.com/rochellemickels/rochelle-2-career-agent)"
TAG_RE = re.compile(r"<[^>]+>")
...
def extract_salary(text: str) -> tuple[int | None, int | None]:
    """Extract an annual USD range without converting hourly compensation."""
    lowered = text.lower()
    if re.search(r"\$\s*\d+(?:\.\d+)?\s*(?:/|per\s+)\s*(?:hour|hr)", lowered):
        return None, None
    ...
    values = [_money_value(value) for value in single_pattern.findall(text)]
    annual = [value for value in values if 40_000 <= value <= 1_000_000]
    if annual:
...
def infer_workplace(location: str, supplied: str | None = None) -> str:
    supplied_lower = (supplied or "").lower()
    location_lower = location.lower()
    if "remote" in supplied_lower or "remote" in location_lower:
        return "Remote"
    if "hybrid" in supplied_lower or "hybrid" in location_lower:
        return "Hybrid"
    if supplied:
    ...
    def collect(self) -> tuple[list[Job], SourceStatus]:
        try:
            ...
            return jobs, SourceStatus(
                ...
                status="ok",
                jobs_found=len(jobs),
                message="Public job feed refreshed successfully.",
            )
...
class AshbySource(BaseSource):
    ...
    def fetch(self) -> list[Job]:
        data = _get_json(
            f"https://api.ashbyhq.com/posting-api/job-board/{self.slug}?includeCompensation=true"
        )
        jobs: list[Job] = []
        for raw in data.get("jobs", []):
            ...
            jobs.append(
                Job(
                    id=f"ashby:{self.slug}:{raw.get('id') or raw.get('jobUrl')}",
                    source_type="ashby",
                    source_slug=self.slug,
                    company=self.company,
                    title=raw.get("title") or "Untitled role",
...
def build_source(config: dict[str, Any]) -> BaseSource:
    ...
    return classes[source_type](config["company"], config["slug"])
